...
PyObject *createNumArrayFromMatDimensions(int dims, const int* sizes, int type)
{
    int depth = CV_MAT_DEPTH(type); //number bits
    int cn = CV_MAT_CN(type); //number channels
...
public:
                  ...
                  uchar*& datastart, uchar*& data, size_t* step)
    {
        ...
        PyObject* o = createNumArrayFromMatDimensions(dims, sizes, type);
...
cv::Mat* NDArrayConverter::toMat(const PyObject *o, void *memory_chunk)
{
    cv::Mat m;
    ...
    //New Mat is pointing to existing Data
    cv::Mat *r;
    if (memory_chunk != 0)
        r = new (memory_chunk) Mat(ndims, size, type, PyArray_DATA(o), step);
    else
        r = new Mat(ndims, size, type, PyArray_DATA(o), step);
...
PyObject* NDArrayConverter::toNDArray(const cv::Mat& m)
{
    ...
    {
        //create numpy-Array-copy of Mat content
